...
import { loadAddonTools, loadAddonWrappers } from "@/common/utils/tools/addonLoader";
...
export interface ToolConfiguration {
  ...
  /** Index signature so addons can carry extra mux-specific fields. */
  [key: string]: unknown;
}
...
export async function getToolsForModel(
  ...
  // Load addon tools (factories registered via src/addons/*/register.ts)
  const addonTools = await loadAddonTools(config);
  ...
  for (const name of Object.keys(addonTools)) {
    allowlistedToolNames.add(name);
  }
  ...
  // Apply addon wrappers (sorted by directory name, then by declaration order).
  // Addon wrappers run before core wrappers so they sit on the inside of the call chain.
  const addonWrappers = await loadAddonWrappers(config);
  for (const { targetTool, apply } of addonWrappers) {
    if (finalTools[targetTool]) {
      finalTools[targetTool] = apply(finalTools[targetTool]);
    }
  }

  // Remove tools disabled by parent (used by subtask spawning to prevent recursion).
  const disabled = config.experiments?.toolPolicy?.disabledTools;
  if (disabled?.length) {
