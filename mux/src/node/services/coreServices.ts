...
import { loadAddonEventHooks } from "@/common/utils/tools/addonLoader";
import type { AddonEvent, AddonEventHelpers } from "@/common/utils/tools/addonRegistry";
...
export function createCoreServices(opts: CoreServicesOptions): CoreServices {
  ...
  // Forward aiService events to addon event hooks
  void loadAddonEventHooks().then((hooks) => {
    if (hooks.length === 0) return;
    const helpers: AddonEventHelpers = {
      nudge: async (workspaceId, message) => {
        try {
          const sendOptions = workspaceService.getGoalContinuationKickoffSendOptions(workspaceId);
          if (!sendOptions) {
            log.debug("Addon nudge: no kickoff options for workspace", { workspaceId });
            return false;
          }
          const result = await workspaceService.sendMessage(
            ...
            message,
            sendOptions,
            { synthetic: true, agentInitiated: true, requireIdle: true, startStreamInBackground: true },
          );
    ...
    const dispatch = (event: AddonEvent) => {
      for (const hook of hooks) {
        Promise.resolve(hook(event, helpers)).catch((err) => {
          log.debug("Addon event hook error", { ...event, error: err as unknown });
        });
      }
    };
    aiService.on("stream-end", (data: StreamEndEvent) =>
      dispatch({ type: "stream-end", workspaceId: data.workspaceId, properties: data }));
    aiService.on("stream-abort", (data: StreamAbortEvent) =>
      dispatch({ type: "stream-abort", workspaceId: data.workspaceId, properties: data }));
    aiService.on("error", (data: ErrorEvent) =>
      dispatch({ type: "error", workspaceId: data.workspaceId, properties: data }));
  }).catch((err) => {
    log.debug("Failed to load addon event hooks", { error: err as unknown });
  });
