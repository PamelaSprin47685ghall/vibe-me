...
import { loadAddonContextInjectors } from "@/common/utils/tools/addonLoader";
...
export class AIService extends EventEmitter {
  ...
  async streamMessage(opts: StreamMessageOptions): Promise<Result<void, SendMessageError>> {
    ...
    try {
      ...
      // Inject addon context (e.g., capitals-context ALL_CAPS.md files)
      if (metadata.projectPath) {
        const injectors = await loadAddonContextInjectors();
        for (const inject of injectors) {
          const addonContext = await inject(metadata.projectPath);
          if (addonContext) {
            workspaceAdditionalSystemContext = workspaceAdditionalSystemContext
              ? `${workspaceAdditionalSystemContext}\n\n${addonContext}`
              : addonContext;
          }
        }
