...
export class TaskService {
  ...
  private async handleStreamEnd(event: StreamEndEvent): Promise<void> {
    ...
    // Skip task-specific stream-end handling for compaction streams.
    // Compaction streams run with tools disabled and are purely internal context
    // management — they must not interfere with task status transitions or recovery logic.
    if (event.metadata.muxMetadata?.type === "compaction-request") {
      return;
    }
  ...
  private async handleTaskStreamError(event: ErrorEvent): Promise<void> {
    ...
    // Compaction streams run with tools disabled — errors during compaction must not
    // trigger task recovery logic. Use the same check as handleStreamEnd when possible.
    const lastMessagesResult = await this.historyService.getLastMessages(workspaceId, 1);
    if (lastMessagesResult.success) {
      const lastMessage = lastMessagesResult.data[0];
      if (lastMessage?.metadata?.muxMetadata?.type === "compaction-request") {
        return;
      }
    }
    ...
    if (
      ...
      const sendResult = await this.workspaceService.sendMessage(
        ...
        "Your background sub-agent task(s) have completed. Use task_await to retrieve their reports and integrate the results.",
        {
          ...
          agentId: resumeOptions.agentId,
          thinkingLevel: resumeOptions.thinkingLevel,
          experiments: parentEntry.workspace.taskExperiments,
        },
        // Skip auto-resume counter reset — this IS an auto-resume, not a user message.
        { skipAutoResumeReset: true, synthetic: true, agentInitiated: true }
      );
