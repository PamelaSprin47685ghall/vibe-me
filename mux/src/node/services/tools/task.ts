...
import { ForegroundWaitBackgroundedError, type TaskCreateArgs } from "@/node/services/taskService";
...
export const createTaskTool: ToolFactory = (config: ToolConfiguration) => {
  return tool({
    ...
    execute: async (args, { abortSignal, toolCallId }): Promise<unknown> => {
      ...
      const { agentId, subagent_type, prompt, title, run_in_background, n, variants, experiments } =
        validatedArgs;
      ...
      for (const launch of taskGroupLaunches) {
        ...
        const finalExperiments =
          experiments != null
            ? (stripNulls(experiments) as TaskCreateArgs["experiments"])
            : config.experiments;
