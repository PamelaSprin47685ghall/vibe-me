...
type LoopState = {
  ...
  rememberAgent: (sessionID: string, agent: unknown) => void;
};
...
async function handleSessionIdle(
  ...
  const { stoppedSessions, sessionAgents, rememberAgent } = state;
...
export function createLoopNudgeHook(ctx: PluginInput) {
  ...
  const rememberAgent = (sessionID: string, agent: unknown): void => {
    if (typeof agent === 'string' && agent) sessionAgents.set(sessionID, agent);
  };
  const state: LoopState = { stoppedSessions, sessionAgents, rememberAgent };
  ...
  return {
    handleEvent: async (input: {
      ...
      if (event.type === 'session.idle') {
        await handleSessionIdle(ctx, state, sessionID);
        return;
      }
